...
import type { DashGincanaDashboard, DashGincanaPremiacao, DashGincanaFiltroFab, DashFabGincanaEstado } from '../utils/dashboard-types';

// ── Helpers ──────────────────────────────────────────────────────────────────

function useContagem(gincanas: DashGincanaDashboard[], onZerou?: () => void) {
    ...
    useEffect(() => {
        ...
        for (const g of gincanas) {
            if (g.fase === 'proxima' && (g.segundos_para_inicio ?? 0) > 0) {
                inicial[g.id] = g.segundos_para_inicio ?? 0;
            }
        }
        setContagem(inicial);
    }, [gincanas]);
...
function formatarHHMM(iso?: string | null) {
    if (!iso) return '';
...
function formatarContagem(seg?: number | null) {
...
function formatarTempoRestante(seg?: number | null) {
    if (!seg || seg <= 0) return '';
    const h = Math.floor(seg / 3600);
    const m = Math.floor((seg % 3600) / 60);
...
function formatarGanhoEm(iso?: string | null) {
    if (!iso) return null;
...
function calcularEstadoFab(gincanas: DashGincanaDashboard[]): DashFabGincanaEstado | null {
...
function maiorPremio(premiacoes?: DashGincanaPremiacao[] | null) {
    if (!premiacoes || premiacoes.length === 0) return null;
    const formatarReais = (v: unknown) => {
    ...
    const comValor = premiacoes.filter(p => parseFloat(String(p.valor_premio_reais ?? '')) > 0);
    if (comValor.length > 0) {
        const maior = comValor.reduce((a, b) =>
            parseFloat(String(b.valor_premio_reais ?? '')) > parseFloat(String(a.valor_premio_reais ?? '')) ? b : a
        );
    ...
    const maior = premiacoes.reduce((a, b) =>
        parseFloat(String(b.meta_valor ?? 0)) > parseFloat(String(a.meta_valor ?? 0)) ? b : a
    , premiacoes[0]);
...
function BarraProgresso({ valor, meta, ganhou }: { valor: number; meta: number; ganhou?: boolean }) {
...
function InfoPagamento({ premioRegistrado, premioPago }: { premioRegistrado?: boolean; premioPago?: boolean }) {
...
function GincanaCardItem({ g, contagemSecs }: { g: DashGincanaDashboard; contagemSecs?: number }) {
    const {
    ...
    const metaAlvo = proxima_meta
        ? parseFloat(String(proxima_meta.meta_valor ?? ''))
        : premiacoes?.length
            ? Math.max(...premiacoes.map(p => parseFloat(String(p.meta_valor ?? p.meta_pontos ?? 0))))
            : 0;
    ...
    if (fase === 'ao_vivo' || fase === 'encerrada_semana') {
        ...
        return (
            <div className={`ds-gincana-card ds-gincana-card--ao-vivo ${ganhou ? 'ds-gincana-card--ganhou' : ''} ${celebrando ? 'ds-gincana-card--celebrando' : ''}`}>
                    ...
                    <div className="ds-gincana-progresso-base">
                        ...
                        {ganhou ? (
                            ...
                            <span className="ds-gincana-status-falta">
                                Faltam {Math.max(0, parseFloat(String(proxima_meta.meta_valor ?? '')) - valorProgresso).toFixed(0)} {unidadeLabel}
                            </span>
                        ) : null}
                    </div>
                </div>
                ...
                {!ehEquipe && minha_posicao && (total_participantes ?? 0) > 0 && (
...
function FiltroVazio({ filtroAtivo, countProxima, setFiltroAtivo }: {
    filtroAtivo: DashGincanaFiltroFab;
    countProxima: number;
    setFiltroAtivo: (f: DashGincanaFiltroFab) => void;
}) {
    // Hierarquia: ao_vivo → proximas → todas
    let icon: string, titulo: string, mensagem: string, linkLabel: string | null = null, linkFiltro: DashGincanaFiltroFab | null = null;
    ...
    return (
        <UIFeedbackNotFound icon={icon} titulo={titulo} mensagem={mensagem}>
            {linkLabel && (
                <button className="ds-filtro-vazio-link" onClick={() => { if (linkFiltro) setFiltroAtivo(linkFiltro); }}>
...
const iconesPorEstado: Record<string, string> = {
    ao_vivo: '🏆',
    proxima: '⏰',
    encerrada: '✓',
};
...
export default function DashFabGincana() {
    const [gincanas, setGincanas] = useState<DashGincanaDashboard[]>([]);
    const [aberto, setAberto] = useState(false);
    // 5.5 — Filtro ativo no bottom sheet (padrão ao_vivo)
    const [filtroAtivo, setFiltroAtivo] = useState<DashGincanaFiltroFab>('ao_vivo');
    ...
    useEffect(() => {
        buscar();
        fetchIntervalRef.current = setInterval(buscar, 10 * 60 * 1000) as ReturnType<typeof setInterval>;
    ...
    const gincanasVisiveis = gincanas.filter(g => {
        ...
        const fim = new Date(g.datetime_fim || 0);
    ...
    function ordenarGincanas(lista: DashGincanaDashboard[]) {
        return [...lista].sort((a, b) => {
            ...
            if (prioA === 2) return new Date(b.datetime_fim || 0).getTime() - new Date(a.datetime_fim || 0).getTime();
            return 0;
        });
    }

    // Gincanas filtradas para exibição no sheet
    let gincanasExibidas: DashGincanaDashboard[];
    switch (filtroAtivo) {
        case 'ao_vivo':
                        ...
                        <div className="ds-fab-gincana-filtros">
                            {filtros.map(f => (
                                <button
                                    ...
                                    onClick={() => setFiltroAtivo(f.id as DashGincanaFiltroFab)}
                                >
                                    {f.label}
                                </button>
                            ))}
