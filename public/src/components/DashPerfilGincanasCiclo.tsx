...
export default function DashPerfilGincanasCiclo() {
    ...
    useEffect(() => {
        fetchAPI('/api/gincanas-pagamentos/meus-premios')
            .then((d: DashMeusPremiosResponse) => {
                // Filtrar apenas prêmios do ciclo atual
                const ciclo = getObjetoCicloCompletoAtual() as { semanas?: Array<{ inicio: string; fim: string }> } | null;
                ...
                const doCiclo = [...(d.pendentes || []), ...(d.pagos || [])].filter(p => {
                    const dt = new Date(p.ganho_em || 0);
    ...
    const totalValor = premios.reduce((sum, p) => sum + (parseFloat(String(p.valor_reais ?? '')) || 0), 0);
        ...
        <div className="perfil-secao">
            ...
            {premios.map(p => (
                <div key={p.id} className="perfil-gincana-item">
                    ...
                    {p.valor_reais && (
                        <span className="perfil-gincana-valor">R$ {parseFloat(String(p.valor_reais ?? '')).toFixed(2)}</span>
                    )}
                </div>
            ))}
            {totalValor > 0 && (
