...
import type { DashAvisoPopup, DashAvisoCor } from '../utils/dashboard-types';
...
interface DashAvisoPopupProps {
    avisos?: DashAvisoPopup[] | null;
    onMarcarVisto: (id: number) => void;
}

export default function DashAvisoPopup({ avisos: avisosIniciais, onMarcarVisto }: DashAvisoPopupProps) {
    const [fila, setFila]             = useState<DashAvisoPopup[]>(avisosIniciais || []);
    const [atual, setAtual]           = useState<DashAvisoPopup | null>(avisosIniciais?.[0] || null);
            ...
            <div className={`dap-card ${semCorpo ? 'dap-card--imagem-full' : ''} ${tipo === 'misto' ? 'dap-card--misto' : ''} ${saindo ? 'dap-card--saindo' : ''} ${entrando ? 'dap-card--entrando' : ''}`}>
                ...
                {(tipo === 'imagem' || tipo === 'misto') && atual.url_imagem && (
                    <div className={imgWrapClass}>
                        <img src={atual.url_imagem} alt={atual.titulo || undefined} className="dap-imagem" />
                        {atual.urgente && (
