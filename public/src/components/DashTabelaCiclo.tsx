...
function fmtDataLonga(dateStr: string) {
    const d = new Date(dateStr + 'T12:00:00');
    return d.toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' });
}

interface DashTabelaCicloProps {
    blocos?: DashBlocoSemana[] | null;
    diasDetalhes?: DashDiaDetalhe[] | null;
    eventosCalendario?: DashEventoCalendario[];
    diasTrabalho?: Record<string, boolean> | null;
}

export default function DashTabelaCiclo({ blocos, diasDetalhes, eventosCalendario = [], diasTrabalho }: DashTabelaCicloProps) {
    const [eventoPopup, setEventoPopup] = useState<{ dateStr: string; eventos: DashEventoCalendario[] } | null>(null);
    ...
    const eventosMap = useMemo(() => {
        const m: Record<string, DashEventoCalendario[]> = {};
    ...
    const folgasSet = useMemo(() => {
        ...
        eventosCalendario.forEach((ev) => {
            if (ev.tipo && TIPOS_FOLGA.has(ev.tipo)) s.add(ev.data);
        });
        return s;
    }, [eventosCalendario]);
    ...
    const ehDiaDeTrabalho = (dow: number, dateStr: string) =>
    ...
    const fmtDiaMes = (dataISO?: string) => {
        if (!dataISO) return '--';
    ...
    const semanasDoCiclo = useMemo(() => {
        ...
        const semanas: Array<Array<{ dateStr: string; diaNum: number; dow: number } | null>> = [];
        let cursor = new Date(inicio);
        let semana: Array<{ dateStr: string; diaNum: number; dow: number } | null> = [];
    ...
    const corBolinha = (dateStr: string | null | undefined, dow: number) => {
        ...
        return (dia.nivelMeta ? CORES_META[dia.nivelMeta as keyof typeof CORES_META] : undefined) || COR_SEM_PROD;
    };
                    ...
                    <div className="ds-cal-popup-content" onClick={e => e.stopPropagation()}>
                        ...
                        <ul className="ds-cal-popup-lista">
                            {eventoPopup.eventos.map((ev, i) => {
                                const info = (ev.tipo ? TIPO_LABEL[ev.tipo] : undefined) || { label: 'Evento', icon: 'fa-calendar-day' };
                                return (
