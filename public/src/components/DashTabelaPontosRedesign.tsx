...
function obterProcessos(produto?: DashTabelaProduto | null): DashTabelaProcesso[] {
    return Array.isArray(produto?.processos) ? produto.processos : [];
}

function obterTotalProduto(produto?: DashTabelaProduto | null) {
...
export default function DashTabelaPontosRedesign({ onClose }: { onClose: () => void }) {
    ...
    useEffect(() => {
        fetchAPI('/api/dashboard/minha-tabela-pontos')
            .then(setDados)
            .catch(() => setDados([] as DashTabelaProduto[]))
