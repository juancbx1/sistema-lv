...
function formatarContagem(segundos: number | null | undefined): string {
...
function fasePretty(fase: string): { label: string; cls: string; dot: boolean } {
    switch (fase) {
...
function participantesPretty(p: string | undefined): string {
    if (p === 'costureiras') return 'Costureiras';
    if (p === 'tiktiks') return 'Tiktiks';
    return 'Costureiras e Tiktiks';
}

function escopoPretty(e: string | undefined, produtoNome: string | null | undefined): string {
...
function tipoMecanicaPretty(tipoPremiacao: string | undefined, modalidade: string | undefined) {
...
function formatarPremioResumo(premiacoes: GincanaPremiacao[] | undefined): string | null {
...
export interface IncenGincanaCardProps {
    gincana: Gincana;
    onEditar: (g: Gincana) => void;
    onPublicar: (g: Gincana) => void;
    onCancelar: (g: Gincana) => void;
    onDeletar: (g: Gincana) => void;
    onVerRanking: (g: Gincana) => void;
}

export default function IncenGincanaCard({
    gincana,
    onEditar,
    onPublicar,
    onCancelar,
    ...
    const tempoRestanteLabel = fase === 'ao_vivo' && gincana.segundos_para_fim != null && gincana.segundos_para_fim > 0
        ? formatarContagem(gincana.segundos_para_fim)
        : fase === 'proxima' && gincana.segundos_para_inicio != null && gincana.segundos_para_inicio > 0
            ? formatarContagem(gincana.segundos_para_inicio)
            : null;

    // Mantém referência da função legada (não usada na UI atual)
    void formatarPremioResumo;
                ...
                <div className="incen-gincana-criterios">
                    {multiNivel ? (
                        <div className="incen-gincana-chips">
                            {premiacoes.map((p, i) => {
                                const vr = parseFloat(String(p.valor_premio_reais ?? ''));
                        ...
                        <>
                            ...
                            <div className="incen-gincana-criterio-bloco">
                                ...
                                <span className="incen-gincana-criterio-valor--premio">
                                    {parseFloat(String(primeiraPremiacao.valor_premio_reais ?? '')) > 0
                                        ? `R$ ${parseFloat(String(primeiraPremiacao.valor_premio_reais)).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
