...
const fmtReal = (valor) => Number(valor || 0).toLocaleString('pt-BR', {
    style: 'currency',
    currency: 'BRL',
});

const nomeMeta = (meta, fallback) => {
    const nome = meta?.descricao_meta?.replace(/^Meta\s+/i, '').trim();
    return nome || fallback;
};

function identificarNivelCenario(meta, indice, total) {
...
export default function DashProjecaoCicloRedesign({
    ...
    const [cenarioSelecionado, setCenarioSelecionado] = useState(metaOuro);
    const [cenarioCelebrando, setCenarioCelebrando] = useState(null);
    const [celebracaoTimer, setCelebracaoTimer] = useState(null);
