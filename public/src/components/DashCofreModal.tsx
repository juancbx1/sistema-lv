...
interface DashCofreModalProps {
    ...
    metaDoDia?: DashMeta | null;
    pontosHoje?: number;
    aoResgatarSucesso: () => void | Promise<void>;
    onClose: () => void;
}

export default function DashCofreModal({ dadosCofre, metaDoDia, pontosHoje, aoResgatarSucesso, onClose }: DashCofreModalProps) {
    const [loading, setLoading] = useState(false);
    const [aba, setAba] = useState<'resumo' | 'extrato' | 'info'>('resumo');
    const [historico, setHistorico] = useState<DashCofreMovimento[]>([]);
    ...
    const saldo          = parseFloat(String(dadosCofre.saldo ?? 0));
    ...
    const pontosHojeNum = Number(pontosHoje || 0);
    const temProducaoMinima  = pontosHojeNum >= PONTOS_MINIMOS;
    const faltaParaMeta      = metaDoDia ? Math.max(0, metaDoDia.pontos_meta - pontosHojeNum) : 0;
    ...
    const progProd = Math.min((pontosHojeNum / PONTOS_MINIMOS) * 100, 100);
    ...
    const carregarExtrato = async (reset = true as boolean) => {
        setLoading(true);
        try {
            const pg = reset ? 1 : paginaExtrato + 1;
            const dados = await fetchAPI(`/api/dashboard/cofre/extrato?page=${pg}&limit=10`) as DashCofreExtratoResponse;
            ...
            <div>
                <div className="ds-cofre-prod-header">
                    ...
                    <span className={`ds-cofre-prod-valor ${temProducaoMinima ? 'ds-cofre-prod-valor--ok' : ''}`}>
                        {Math.round(pontosHojeNum)} / {PONTOS_MINIMOS} pts
                    </span>
                </div>
                ...
                <p className="ds-cofre-prod-hint">
                    {temProducaoMinima
                        ...
                        : `Faltam ${PONTOS_MINIMOS - Math.round(pontosHojeNum)} pts para liberar o resgate`}
                </p>
            </div>
    ...
    const renderExtrato = () => {
        ...
        const grupos: Array<{ tipo: string; item?: DashCofreMovimento & { saldoApos?: number | null }; dataStr?: string }> = [];
                            ...
                            return (
                                <div key={`i${idx}`} className="ds-cofre-extrato-item">
                                    ...
                                    <span className={`ds-cofre-extrato-valor ${isGanho ? 'ds-cofre-extrato-valor--ganho' : 'ds-cofre-extrato-valor--resgate'}`}>
                                        {isGanho ? '+' : '−'}{Math.round(Number(item.quantidade))} pts
                                    </span>
                                </div>
                            );
                        })
                ...
                <div className="ds-cofre-tabs" role="tablist">
                    {[
                        ...
                        <button
                            ...
                            onClick={() => t.id === 'extrato' ? handleAbaExtrato() : setAba(t.id as 'resumo' | 'extrato' | 'info')}
                        >
                            {t.label}
                        </button>
                    ))}
