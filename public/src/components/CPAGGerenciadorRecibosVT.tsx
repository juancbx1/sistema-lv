...
export default function CPAGGerenciadorRecibosVT({ isOpen, onClose }: Props) {
  ...
  const carregarDados = async () => {
    ...
    try {
      const [lotesData, usuario] = await Promise.all([
        ...
        fetchCpag<CpagUsuario>('/api/usuarios/me'),
      ]);
  ...
  const gerarPDFLote = async (lote: CpagLoteVT) => {
    try {
      await fetchCpag('/api/pagamentos/marcar-lote-impresso', {
        method: 'POST',
        body: JSON.stringify({ ids: lote.itens.map((item) => item.id) }),
      });
      setLotes((atual) =>
        atual.map((item) =>
          item.data_pagamento === lote.data_pagamento && item.descricao === lote.descricao
            ? { ...item, ja_impresso: true }
            : item,
        ),
      );
      ...
      lote.itens.forEach((item, index) => {
        ...
        try {
          detalhes =
            typeof item.detalhes === 'string'
              ? (JSON.parse(item.detalhes) as { datas_pagas?: string[] })
              : item.detalhes ?? {};
        } catch {
          detalhes = {};
        }
        ...
        doc.setFontSize(10);
        doc.text(
          `Data do Pagamento: ${new Date(lote.data_pagamento).toLocaleDateString('pt-BR')}`,
          14,
          30,
        );
        doc.text(`Empregado: ${item.nome_funcionario}`, 14, 36);
        autoTable(doc, {
          startY: 45,
          head: [['Referente ao Dia', 'Valor Repassado']],
          body: datasPagas.map((data) => [
        ...
        doc.text(item.nome_funcionario, 105, finalY + 5, { align: 'center' });
        doc.text('Assinatura do Empregado', 105, finalY + 10, { align: 'center' });
        doc.setFontSize(7);
        doc.text(
          `Gerado em ${new Date().toLocaleString('pt-BR')} por ${nomeAdmin}`,
          14,
      ...
      console.error(error);
      mostrarToast(error instanceof Error ? error.message : 'Erro ao gerar PDF.', 'erro');
    }
  };
            ...
            <table className="cpg-tabela-detalhes">
              ...
              <tbody>
                {lotesVisiveis.map((lote) => (
                  <tr key={`${lote.data_pagamento}-${lote.descricao}`}>
                    <td>{new Date(lote.data_pagamento).toLocaleDateString('pt-BR')}</td>
                    <td>{lote.descricao}</td>
                    <td>{lote.qtd_funcionarios}</td>
