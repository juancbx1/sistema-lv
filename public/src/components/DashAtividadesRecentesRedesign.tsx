...
function deslocarDataISO(dataISO: string, quantidade: number) {
...
function calcularDataAlvo(filtroPeriodo: DashFiltroPeriodoAtividades, dataEspecifica: string) {
...
function descreverPeriodo(filtroPeriodo: DashFiltroPeriodoAtividades, dataEspecifica: string, termoBusca: string) {
    const hoje = dataHojeISO();
...
function formatarDataAtividade(data?: string | null) {
...
function formatarHoraAtividade(data?: string | null) {
    const convertido = converterData(data);
    if (!convertido || Number.isNaN(convertido.getTime())) return '--:--';
    return new Intl.DateTimeFormat('pt-BR', {
        timeZone: TIME_ZONE,
...
function formatarPontos(valor?: number | string | null) {
...
export default function DashAtividadesRecentesRedesign() {
    ...
    const selecionarData = (event: React.ChangeEvent<HTMLInputElement>) => {
                        ...
                        <article key={chave} className={`ds-atividades-redesign-evento${extra ? ' ds-atividades-redesign-evento--extra' : ''}`}>
                            ...
                            <div className="ds-atividades-redesign-evento-corpo">
                                <div className="ds-atividades-redesign-evento-topo">
                                    ...
                                    <time dateTime={item.data || undefined}>{formatarHoraAtividade(item.data)}</time>
                                </div>
                                <h3>{nomeProduto}</h3>
                                {extra ? (
