...
export default function DashRankingCard() {
    const [dados, setDados] = useState<DashRankingSemana | null>(null);
    ...
    useEffect(() => {
        buscar();
        intervalRef.current = setInterval(buscar, 10 * 60 * 1000) as ReturnType<typeof setInterval>;
    ...
    const maxPontos = Math.max(...ranking.filter(r => !r.separador && r.pontos != null).map(r => r.pontos as number), 1);

    const gerarLabel = (item: DashRankingItem) => {
