...
function obterNome(item, tipoUsuario) {
    if (item?.isEu) return 'Você';
    return tipoUsuario === 'tiktik' ? `Tiktik #${item?.posicao || '—'}` : `Colega #${item?.posicao || '—'}`;
}

function obterMotivacao(dados, campea) {
    if (dados.todosZerados) return 'A semana está começando. Cada etapa já conta.';
    if (campea) return dados.semanasNoTopo > 1
        ? `${dados.semanasNoTopo} semanas seguidas no topo.`
        : 'Você está liderando a semana.';
    if (dados.gapParaProximo <= 50) return `Só ${dados.gapParaProximo} pts para subir.`;
    if (dados.gapParaProximo <= 200) return `${dados.gapParaProximo} pts para alcançar a próxima posição.`;
    return `Continue produzindo para se aproximar da próxima posição.`;
}

export default function DashRankingMenu({ dados, variante = 'mobile' }) {
    const [slide, setSlide] = useState(0);
    const toqueInicial = useRef(null);

    if (!dados || dados.totalParticipantes <= 1 || !dados.ranking?.length) return null;
