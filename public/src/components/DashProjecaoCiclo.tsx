...
import type { DashMeta, DashDiaDetalhe } from '../utils/dashboard-types';

const fmtReal = (v: number) => v.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

interface DashProjecaoCicloProps {
...
export default function DashProjecaoCiclo({
    ...
    diaHojeJaEncerrado,
    aoAbrirWallet,
}: DashProjecaoCicloProps) {
    const bronzePontos = Number(metasPossiveis[0]?.pontos_meta || 0);
    const prataPontos  = Number(metasPossiveis[1]?.pontos_meta || 0);
    const ouroPontos   = Number(metasPossiveis[metasPossiveis.length - 1]?.pontos_meta || 0);
    const valorOuro    = Number(metasPossiveis[metasPossiveis.length - 1]?.valor_comissao || 0);

    // Dias restantes — usa o valor computado pela API (mais preciso) com fallback
    const diasRestantes = diasRestantesNoCiclo ?? Math.max(0, (diasUteisNoCiclo ?? 0) - (diasTrabalhadosNoCiclo ?? 0));
    ...
    const nivelAtual = useMemo(() => {
        ...
        const freq = ultimos5.reduce<Record<string, number>>((acc, d) => {
            const pts = d.pontos;
    ...
    const labelDias = (n: number) => n === 1 ? '1 dia' : `${n} dias`;
    ...
    const renderCorpo = () => {
            ...
            return (
                <>
                    ...
                    <div className="ds-projecao-nivel-row">
                        <span className="ds-projecao-nivel-icone">ðŸ’°</span>
        ...
        if (estado === 'ciclo-novo') {
            const potencial = valorOuro * (diasUteisNoCiclo ?? 0);
            return (
                <>
