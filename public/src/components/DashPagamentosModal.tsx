...
function formatarPeriodo(isoInicio?: string | null, isoFim?: string | null) {
...
function WalletTopo({ onClose, abaAtiva, acumuladoCicloAtual, saldoPremiacoes, loadingPremiacoes }: {
    onClose: () => void;
    abaAtiva: 'comissoes' | 'premiacoes' | string;
    acumuladoCicloAtual?: DashCicloPagamentoResumo | null;
    saldoPremiacoes?: number;
    loadingPremiacoes?: boolean;
}) {
    const valorCom = parseFloat(String(acumuladoCicloAtual?.valor ?? '')) || 0;
    const valorPre = saldoPremiacoes || 0;
    const periodoAtual = acumuladoCicloAtual
        ? formatarPeriodo(acumuladoCicloAtual.periodoInicio ?? null, acumuladoCicloAtual.periodoFim ?? null)
        : '';
    ...
    const abaAnterior = useRef<string | null>(null); // null = primeira execução

    const animarContador = useCallback((setDisplay: React.Dispatch<React.SetStateAction<number>>, alvo: number, duracao = 550) => {
...
function ProxPagamentoCard({ pagamentoCicloFechado }: { pagamentoCicloFechado?: DashCicloPagamentoResumo | null }) {
    const valor = parseFloat(String(pagamentoCicloFechado?.valor ?? '')) || 0;
...
// ─── ABA COMISSÕES ─────────────────────────────────────────────────────────
function fmtMesAno(iso?: string | Date | null) {
    if (!iso) return '';
    return new Date(iso as string | Date).toLocaleDateString('pt-BR', {
...
function AbaComissoes({ pagamentoCicloFechado, historico, loading }: {
    ...
    historico: DashComissaoHistoricoItem[];
    loading: boolean;
}) {
    ...
    const grupos = useMemo(() => {
        ...
        const sorted = [...historico].sort(
            (a, b) => new Date(b.data_pagamento || 0).getTime() - new Date(a.data_pagamento || 0).getTime()
        );
        const result = [];
    ...
    const labelPeriodo = useMemo(() => {
        ...
        const datas = grupoAtual.map(p => new Date(p.data_pagamento || 0));
                        ...
                        <div key={idx} className="ds-pag-historico-linha">
                            ...
                            <div className="ds-pag-historico-direita">
                                <div className="ds-pag-historico-valor">
                                    R$ {parseFloat(String(pgto.valor_liquido_pago ?? '')).toFixed(2)}
                                </div>
                                ...
                                {pagosPagina.map(p => (
                                    <div key={p.id} className="ds-pag-historico-linha">
                                        ...
                                        <div className="ds-pag-historico-direita">
                                            {p.valor_reais && (
                                                <div className="ds-pag-historico-valor">
                                                    R$ {parseFloat(String(p.valor_reais ?? '')).toFixed(2)}
                                                </div>
                                            )}
...
// ─── MODAL PRINCIPAL ──────────────────────────────────────────────────────
interface DashPagamentosModalProps {
    acumuladoCicloAtual?: DashCicloPagamentoResumo | null;
    pagamentoCicloFechado?: DashCicloPagamentoResumo | null;
...
export default function DashPagamentosModal({ acumuladoCicloAtual, pagamentoCicloFechado, usuario, onClose }: DashPagamentosModalProps) {
    const [aba, setAba] = useState<'comissoes' | 'premiacoes'>('comissoes');
    const [historicoCom, setHistoricoCom] = useState<DashComissaoHistoricoItem[]>([]);
    const [loadingCom, setLoadingCom] = useState(true);
    const [dadosPre, setDadosPre] = useState<DashMeusPremiosResponse>({ pendentes: [], pagos: [] });
    const [loadingPre, setLoadingPre] = useState(true);

    useEffect(() => {
        fetchAPI('/api/dashboard/meus-pagamentos')
            .then((d: unknown) => setHistoricoCom(d as DashComissaoHistoricoItem[]))
            .catch(() => setHistoricoCom([]))
            .finally(() => setLoadingCom(false));

        fetchAPI('/api/gincanas-pagamentos/meus-premios')
            .then((d: unknown) => setDadosPre(d as DashMeusPremiosResponse))
            .catch(() => setDadosPre({ pendentes: [], pagos: [] }))
            .finally(() => setLoadingPre(false));
    }, []);

    const saldoPremiacoes = (dadosPre.pendentes || [])
        .reduce((sum, p) => sum + (parseFloat(String(p.valor_reais ?? '')) || 0), 0);
