...
import type { CpagContaFinanceira, CpagTab, CpagUsuario } from '../utils/cpag-types';

interface Props { permissoes: string[]; }
...
export default function CPAGCentralPagamentos({ permissoes }: Props) {
    ...
    const carregarDadosIniciais = async () => {
      try {
        const [usersData, finData] = await Promise.all([
          ...
          fetchCpag<{ contas?: CpagContaFinanceira[] }>('/api/financeiro/configuracoes'),
        ]);
        if (!ativo) return;
        const elegiveis = (Array.isArray(usersData) ? usersData : [])
          .filter((usuario) => usuario.elegivel_pagamento === true && Boolean(usuario.data_admissao) && !usuario.data_demissao)
          .sort((a, b) => a.nome.localeCompare(b.nome, 'pt-BR'));
        setUsuarios(elegiveis);
        setContasFinanceiras(finData.contas ?? []);
      } catch (error) {
        if (ativo) setErro(error instanceof Error ? error.message : 'Erro ao carregar dados do sistema.');
      } finally {
  ...
  if (loading) return <UICarregando variante="pagina" tamanho="lg" texto="Carregando dados..." />;
  if (erro) return <div className="cpg-card" role="alert"><h2>Não foi possível carregar a Central de Pagamentos</h2><p>{erro}</p><button type="button" className="cpg-btn cpg-btn-primario" onClick={() => window.location.reload()}>Tentar novamente</button></div>;
  ...
  return (
    <>
      ...
      <div className="gs-conteudo-pagina">
        ...
        {activeTab === 'beneficios' && <CPAGBeneficios usuarios={usuarios} contas={contasFinanceiras} />}
      </div>
